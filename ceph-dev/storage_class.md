...
```
make ceph_test_cls_rgw_opstate -j8
...
TEST  (test1, run1) {
  ...
  store->get_bucket_info(obj_ctx, "", "test1", bucket_info, NULL, NULL);
...
./bin/ceph_test_cls_rgw_opstate -g 127.0.0.1 -p 80 -c ceph.conf -rgw-admin ./bin/radosgw-admin --gtest_filter=test1.run1
```
